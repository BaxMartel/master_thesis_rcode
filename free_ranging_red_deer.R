...
df1 <- st_read("YOUR/PATH/Collar16984_GPS_Fischhorn_Rotwild.kml"); df1$id <- 16984
df2 <- st_read("YOUR/PATH/Collar16988_GPS_Fischhorn_Rotwild.kml"); df2$id <- 16988
...
dfges <- subset(dfges, select = c( - Description, - geometry, - date, - time, -z))


## SAFE PROCESSED DATA

write.csv2(dfges,"YOUR/PATH/free_ranging_red_deer.csv")
...
write.csv2(dist,"YOUR/PATH/distance_free_ranging_red_deer.csv")
